...
const Cases = () => {
  ...
  const renderCaseCard = (caseItem: CaseItem) => (
    <View key={caseItem.id} className="bg-white rounded-lg p-4 mb-4 shadow-sm border border-gray-100">
      <Text className="text-lg font-semibold text-gray-800 mb-2">{caseItem.title}</Text>
      
      <View className="flex-row justify-between mb-3">
        <View className="flex-1 mr-4">
          <Text className="text-sm text-gray-500 mb-1">Case Owner</Text>
          <Text className="text-sm font-medium text-gray-800">{caseItem.caseOwner}</Text>
        </View>
        <View className="flex-1">
          <Text className="text-sm text-gray-500 mb-1">Case Type</Text>
          <Text className="text-sm font-medium text-gray-800">{caseItem.caseType}</Text>
        </View>
      </View>
      ...
      <View className="flex-row justify-between mb-3">
        <View className="flex-1 mr-4">
          ...
          <Text className="text-sm font-medium text-gray-800">{caseItem.nextHearing}</Text>
        </View>
        <View className="flex-1">
          <Text className="text-sm text-gray-500 mb-1">Associates</Text>
          <Text className="text-sm font-medium text-gray-800">{caseItem.associates.join(", ")}</Text>
        </View>
      </View>
      ...
      <View className="flex-row justify-between mb-4">
        <View className="flex-1 mr-4">
          ...
          <Text className={`text-sm font-medium ${getStatusColor(caseItem.status)}`}>
            {caseItem.status}
          </Text>
        </View>
        <View className="flex-1">
          <Text className="text-sm text-gray-500 mb-1">Amount</Text>
          <Text className="text-sm font-medium text-gray-800">{caseItem.amount}</Text>
        </View>
      </View>
      ...
      <View className="flex-row justify-between">
        ...
        >
          <Text className="text-center text-gray-700 font-medium">Close Case</Text>
        </TouchableOpacity>
        ...
        >
          <Text className="text-center text-white font-medium">View Details</Text>
        </TouchableOpacity>
      </View>
    </View>
  );
  ...
  return (
    <View className="flex-1 bg-gray-50">
      {/* Header */}
      <View className="flex-row justify-between items-center px-5 pt-12 pb-4 bg-white border-b border-gray-200">
        <TouchableOpacity>
          <Ionicons name="menu-outline" size={24} color="black" />
        </TouchableOpacity>
        <View className="items-center">
          <Text className="text-xl font-semibold">Case Profiles</Text>
        </View>
        <TouchableOpacity>
